...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        # 2. compute the eigenvalues and eigenvectors of the covariance matrix
        e_vals, e_vecs = eig(CovM)
        
        # 3. Sort both of them in decreasing order (ex: 1.0 > 0.5 > 0.0 > -0.2 > -1.2)
        sorted_index = np.flip(np.argsort(e_vals))
        e_vals = e_vals[sorted_index]
        e_vecs = np.real(e_vecs[:, sorted_index])
    ...
    def transform(self, X, n_components):
        ...
        # 2. then project onto a subspace of dimension `n_components` using `self.W_pca`
        X_proj = X_centered @ self.W_pca[:,:n_components]  # (N*D) * (D*n_components)
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        # Sb*wi = lambda*Sw*wi  <===>  A*wi = lambda*B*wi  ==> A = Sb, B = Sw
        e_vals, e_vecs = eig(scatter_between, scatter_within)
