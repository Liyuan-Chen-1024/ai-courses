...
def hog_feature(image, pixel_per_cell=8):
    ...
    """
    hog_feature, hog_image = feature.hog(image, 
...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    # default value for mode='constant' is 0
    pad_image = np.pad(
        image,
        ((winH // 2,
          winH - winH // 2),
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            win_image = pad_image[r:r+winH, c:c+winW]
            win_hog_feature, _ = hog_feature(win_image, pixel_per_cell)
            score = win_hog_feature.dot(template_feature)
            if score > max_score:
                max_score = score
                maxr = r
                maxc = c
...
def pyramid(image, scale=0.9, min_size=(200, 100)):
    ...
    while True:
        ...
        if current_size[0] < min_size[0] or current_size[1] < min_size[1]:
            break
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    for scale, image in images:
        score, r, c, response_map_resized = sliding_window(image, template_feature, step_size, shape, pixel_per_cell)
        if score > max_score:
            max_score = score
            maxr = r
            maxc = c
...
#           max_response_map = response_map_resized
            max_response_map = resize(response_map_resized, image.shape, mode='constant')
...
def compute_displacement(part_centers, face_shape):
    ...
    # face's center-x coordinate minus the ith part's x coordinate
    d[:,0] = face_center[0] - part_centers[:,0]  # shift distance in x direction
    d[:,1] = face_center[1] - part_centers[:,1]  # shift distance in y direction
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    assert len(heatmaps) == len(sigmas)
    
    heatmap = np.zeros((heatmap_face.shape))
    
    for i in range(len(heatmaps)):
        part_heatmap = heatmaps[i]
        part_sigma = sigmas[i]
        
        heatmap += gaussian(part_heatmap, sigma = part_sigma)
...
def detect_multiple(image, response_map, face_shape):
    ...
    for i in range(4):
        ...
        response_map[maxr-winH//2: maxr+winH//2, maxc-winW//2:maxc+winW//2] = 0
