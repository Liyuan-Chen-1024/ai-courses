...
#     for i in range(M):
#         for j in range(N):
#             dists[i, j] = np.sqrt(np.sum(np.square(X1[i,:] - X2[j,:])))
            
    X1X1 = np.sum(X1**2, axis=1).reshape(M, 1)    # shape (M, 1)
    X1X1 = np.repeat(X1X1, N, axis=1)             # shape (M, N)
    
    X2X2 = np.sum(X2**2, axis=1).reshape(1, N)    # shape (1, N)
    X2X2 = np.tile(X2X2, (M, 1))                  # shape (M, N)    
    
    X1X2 = X1 @ X2.T                              # shape (M, N)
    
    dists = np.sqrt(X1X1 + X2X2 - 2*X1X2)
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        ybin = np.bincount(closest_y, minlength=num_train)
        y_pred[i] = np.argmax(ybin)
...
def split_folds(X_train, y_train, num_folds):
    ...
    # note: X_train different from X_trains
    X_pieces = np.array_split(X_train, num_folds)   # return list of ndarray
    y_pieces = np.array_split(y_train, num_folds)
    
    for i in range(num_folds):
        X_pieces_tmp = X_pieces.copy()
        y_pieces_tmp = y_pieces.copy()
            
        X_vals[i] = X_pieces_tmp[i]
        y_vals[i] = y_pieces_tmp[i]
        
        del X_pieces_tmp[i]  # del list[idx]
        del y_pieces_tmp[i]
        
        X_trains[i] = np.concatenate(X_pieces_tmp)
        y_trains[i] = np.concatenate(y_pieces_tmp)
